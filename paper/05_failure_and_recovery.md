...
An admission decision is a function of the order, the gateway's own state, and
the ceilings it held. Row 10 of the NFR table requires any decision to be
replayable. §2.7 lists what that puts on the log and why the ceilings themselves
are derived rather than logged.

This is a stronger requirement than the running case has, because three
components must reconstruct the same facts without reaching each other. Two
consequences carry into the rest of this section: **a snapshot bounds replay time
and is never a correctness requirement**, and **replay is idempotent by log
position** rather than by order ID — which covers admissions but not fills or
cancels, and r2 caught that.
...
- **a fill** is refused, writing nothing, on a reused identifier with different
  ...
  price outside the band recorded at admission, or a fee above the cap;
...
**The cost is that the venue is the counterparty.** Risk moves from the account
to the venue's own book and, past the account's equity, to the insurance fund.
Venue-side portfolio risk and capital limits for that book are not modelled
...
matches. The figures come from the log, not from whoever asks.
...
still looks fully occupied. The account-wide path uses no seal, because the
allocator reads the same log the seal was cut from, so an undelivered seal does
not freeze capacity.
...
reachable position **at the moment authority ends**, and outside any bound taken
when the shortfall was detected, because everything admitted during the detection
delay is inside the first and outside the second. In the unfenced arm no such
moment exists and the cost exceeds the bound by 1,562.
...
| Event | Inputs | Conclusion |
|---|---|---|
| Warm failover | Followers apply the same log and hold the same books; cost is election plus commit catch-up on the tail | Inside the 3 s budget of row 8. During the window gateways keep their ceilings and admit inside them; when terms expire they admit nothing at all, and risk reduction is the liquidation path's job (§3.3) — so a failover longer than the shortest outstanding term leaves accounts with no ingress |
| Cold rebuild | 187 MB of snapshot from NVMe; 5 min of log tail at ≈ 21 MB/s (§2.7); replay assumed at 10× live | Inside the 60 s budget. The replay rate is the one assumed figure in this arithmetic and is the first chaos experiment (§8.1) |
| Correctness of either | Snapshot plus replay compared against a gateway built from the whole log | E4: 3,642 injected crashes, zero equivalence failures, including 2,364 mid-partial-fill and 819 stale-snapshot cases |

## 5.6 Zero-downtime upgrade

Anything a decision is derived from is versioned data on the log, activated at a
sequence (§4.1), so a replay of an old decision uses the values in force then.
Upgrades are therefore rolling with the state machine version pinned per record,
and an operator cannot tune a derivation mid-session. That is a real operational
loss and it is the price of NFR row 10.
...
auto-deleveraging, and who absorbs a shortfall the fund cannot; the replay-rate
assumption of §5.5; and cross-datacentre replication.
